...
d_trim <- d %>% 
  ...
  filter(complete.cases(.)) %>% # could have also used ?drop_na
  rename(loc_lat = location_latitude, #nice code style
         ...
         basic_8 = q25_8, 
         higherorder_1 = q24_1, 
         #for these groups of columns (higherorder, persmanage, etc.), you could use: rename_at(vars(contains("higherorder)),funs(str_replace(., "q24", "higherorder)))
         higherorder_2 = q24_2,  
         higherorder_3 = q24_3,            
         higherorder_4 = q24_4, 
         higherorder_5 = q24_5, 
...
centroids <- import(here("data", "centroids.csv"), 
                    setclass = "tbl_df") %>% 
  mutate(region = tolower(name)) #nice!
...
#You could pipe lines 224-230 rather than making two objects
centroids <- left_join(centroids, counts_state) %>% 
  mutate(n = ifelse(is.na(n), 0, n))
...
results <- apa_print(fit) #nice use of apa_print
...
d_trim <- d_trim %>% 
  drop_na() %>% 
# Nice code below! I gave you an extra point for your nice use of mutate and their helper functions.
...
higherorder <- d_trim %>% 
  select(contains("higherorder")) %>% #good helper function
  rename(
...
<!-- I changed the figure width here in the chunck options to help the fig fit. -->
```{r pivotlonggganimate, fig.cap = "Interpersonal Skills", fig.width=8.5 }
...
#This is great code! It incorporated so many of the things we learned this term. Be careful here because the industry names are long and are thus cut off in your final paper. I took a point off for this.
d_long %>% 
...
# You can use regular expression to add a new line (\n) to these labels to make more room. See below.  
  scale_x_discrete(labels = c("1" = "Use Socially\n Acceptable Language",
                            "2" = "Show Respect\n for Others",
                            "3" = "Work Well with Individuals\n from Diverse Backgrounds",
                            "4" = "Cooperate with Others",
                            "5" = "Model Integrity and\n Ethical Leadership",
                            "6" = "Advocate for Self",
                            "7" = "Negotiate and\n Resolve Conflict",
...
x = "",
y = "",
fill = "") # I removed the legend title because it was not giving information.
```
...
<!-- I took a point from the Method because I am used to seeing a description of the current study before the Method section. It helps set the context for the research questions.-->
# Methods
## Research Questions:  
...
to complete.
<!-- If you do publish based on this study, it would be nice to include a copy of the survey in the Appendix. See https://crsh.github.io/papaja_man/writing.html#appendices -->
...
<!-- You can use headers, which would help organize the paper. Here, for instance, you could include ## Analyses -->
Given the
...
# Results 
<!-- The percentages reported in paragraph below would have been perfect for inline code. -->
